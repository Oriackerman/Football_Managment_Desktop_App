...
using Football_Managment.Models;
...
namespace Football_Managment
{
    public partial class MainApp : Form
    {
        ...
        private void btn_backUp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!Directory.Exists(backUpPath))
                {
                    ...
                    MessageBox.Show("The path for backup doesn't exist: " + backUpPath,
                          "Failed to backUp",
                          MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                string mongoDumpFile = Path.Combine(backUpPath, "mongodump.exe");
                if (!File.Exists(mongoDumpFile))
                {
                    MessageBox.Show("The mongodump executable doesn't exist in the backup path: " + mongoDumpFile,
                            "Failed to backUp",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                process.Start();
                process.StandardInput.WriteLine("@cd " + backUpPath);
                process.StandardInput.WriteLine("@" + backUpLocation);
                process.StandardInput.Close();
                string output = process.StandardOutput.ReadToEnd();
                process.WaitForExit();
                MessageBox.Show("Back up Succeeded !");
            }
            catch (Exception ex)
            {
                MessageBox.Show("The back up wasn't Succeessful : " + ex.Message);
            }
        }
        ...
        private void btn_restore_Click_1(object sender, EventArgs e)
        {
            try
            {
                string restorePath = ConfigurationManager.AppSettings["pathForRestore"];
                string restoreLocation = ConfigurationManager.AppSettings["pathForRestoreLocation"];

                if (!Directory.Exists(restorePath))
                {
                    MessageBox.Show("The path for restore doesn't exist: " + restorePath,
                                    "Failed to Restore",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                string mongoRestoreFile = Path.Combine(restorePath, "mongorestore.exe");
                if (!File.Exists(mongoRestoreFile))
                {
                    MessageBox.Show("The mongorestore executable doesn't exist in the restore path: " + mongoRestoreFile,
                                    "Failed to Restore",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                string restoreDirectory = restoreLocation.Split(' ').Last();
                if (!Directory.Exists(restoreDirectory))
                {
                    MessageBox.Show("The restore directory doesn't exist: " + restoreDirectory,
                                    "Failed to Restore",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                string databaseDirectory = restoreDirectory;
                if (!Directory.Exists(databaseDirectory))
                {
                    MessageBox.Show("The TeamsPlayersContracts directory doesn't exist in the restore directory: " + databaseDirectory,
                                    "Failed to Restore",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                process.StartInfo = startInfo;
                process.Start();
                process.StandardInput.WriteLine("@cd " + restorePath);  // MongoDB bin directory
                ...
                if (process.ExitCode != 0)
                {
                    ...
                    MessageBox.Show("Restore operation failed with error code: " + process.ExitCode,
                                    "Failed to Restore",
                                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
